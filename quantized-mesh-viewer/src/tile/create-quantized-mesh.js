...
function loadMesh(path) {
  if (path.startsWith("http")) {
    ...
    return axios.get(path, { headers, responseType: 'arraybuffer' })
      .then(response => {
        // const buffer = new Uint8Array(Object.values(response.data));
      ...
      .catch(error => {
        ...
        throw error; // 抛出错误，以便调用者能够处理错误情况
      });
...
function constructNormalAttribute(vertexNormalsBuffer, vertexData) {
  vertexNormalsBuffer = new Uint8Array(Object.values(vertexNormalsBuffer)).buffer;
  console.log('vertexNormalsBuffer', typeof vertexNormalsBuffer, vertexNormalsBuffer);
...
export default function createQuantizedMesh(scene, container, params) {
  ...
  return loadMesh(params.meshUrl)
    ...
    .then(decodedMesh => {
      console.log('decodedMesh', decodedMesh)
